...
import { IBaseService, PaginationQuery } from './base.service.interface';
...
> {
  ...
  @Delete('bulk/delete')
  @HttpCode(HttpStatus.NO_CONTENT)
  async deleteMany(@Body() body: { ids: string[] }): Promise<void> {
    if (!body.ids || body.ids.length === 0) {
      throw new NotFoundException('No IDs provided for deletion');
    }
  ...
  @Post('bulk/create')
  @HttpCode(HttpStatus.CREATED)
  async createMany(@Body() body: { items: TCreateDto[] }): Promise<TResponseDto[]> {
    if (!body.items || body.items.length === 0) {
      throw new NotFoundException('No items provided for creation');
    }

    const entities = await this.service.createMany(body.items);
    return this.toResponseDtoList(entities);
  }

  @Put('bulk/update')
  async updateMany(
    @Body() body: { updates: Array<{ id: string; data: TUpdateDto }> },
  ): Promise<TResponseDto[]> {
